...
- **Known Pitfalls**: `fl_effect` and `fl_plugin` are annotated `write-safe-required` because each public function can perform persistent writes; individual action safety is enforced by registry dispatch and documented in the tool docstrings. Plugin loading, plugin insertion, plugin removal, preset navigation writes, and full effect-chain restore remain unsupported. Native EQ type writes remain documented-unconfirmed on current live evidence and should only be trusted where readback verifies the change.
