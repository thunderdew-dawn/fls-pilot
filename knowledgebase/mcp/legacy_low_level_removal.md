...
- **Affected File/API**: `src/fl_studio_mcp/server.py`, `scripts/check_tool_registration_baseline.py`, domain MCP tools
...
- **Observation**: The public FastMCP surface now registers 86 tools with 86 unique public names. Redundant aliases covered by `fl_transport`, `fl_mixer`, `fl_channel`, `fl_pattern`, `fl_playlist`, `fl_effect`, `fl_plugin`, `fl_piano_roll`, and `fl_batch` are removed without deprecation wrappers.
- **Tested Values**: Registration baseline: 86 public tools. Safety classes: 33 `write-safe`, 40 `read-only`, 4 `server-state`, 2 `external-write`, 7 unannotated Knowledgebase tools. Focused domain parity suite: 58 tests.
