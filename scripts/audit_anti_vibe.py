...
def report_error(filepath, line, violation_type, suggestion):
    errors.append(f"{RED}[VIBE CODING] {filepath}:{line}{RESET} - {YELLOW}{violation_type}{RESET} -> {suggestion}")

# --- 1. Knowledgebase Compliance ---
REQUIRED_KB_FIELDS = {
    "date", "topic", "context", 
    "observation", "tested_values", "result", "confidence_level", 
    "source_method", "open_questions", "next_recommended_action"
}
# author / agent_author and affected_api / affected_file_api are handled dynamically

ALLOWED_CONFIDENCE_LEVELS = {
    "hypothesis", "user_reported", "docs_confirmed", "measured_once", 
    "measured_repeated", "implementation_verified", "cross_platform_verified", 
        ...
        try:
            ...
            for i, entry in enumerate(entries):
                ...
                if conf and conf not in ALLOWED_CONFIDENCE_LEVELS:
                    report_error(
                        rel_path, 1, "Invalid Confidence Level", 
...
class SandboxVisitor(ast.NodeVisitor):
    ...
    def visit_Import(self, node):
        for alias in node.names:
            ...
            if base_module in FORBIDDEN_SANDBOX_MODULES:
                report_error(
                    self.rel_path, node.lineno, "Sandbox Violation (Import)", 
    ...
    def visit_ImportFrom(self, node):
        if node.module:
            ...
            if base_module in FORBIDDEN_SANDBOX_MODULES:
                report_error(
                    self.rel_path, node.lineno, "Sandbox Violation (ImportFrom)", 
    ...
    def visit_Call(self, node):
        if isinstance(node.func, ast.Name) and node.func.id == "open":
            report_error(
                self.rel_path, node.lineno, "Sandbox Violation (open() call)", 
...
def check_controller_sandbox():
    ...
    for py_file in CONTROLLER_DIR.rglob("*.py"):
        ...
        try:
            ...
            report_error(rel_path, e.lineno, "Syntax Error", "Fix Python syntax so AST can parse it.")
        ...
        for py_file in d.rglob("*.py"):
            ...
            with open(py_file, "r", encoding="utf-8") as f:
                for i, line in enumerate(f, 1):
                    ...
                    if match:
                        report_error(
                            rel_path, i, f"Vibe Keyword Detected ('{match.group(1)}')", 
