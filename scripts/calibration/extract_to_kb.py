...
def main():
    ...
    if eq_form_path.exists():
        ...
        gain_mappings = [
            {"normalized": 0.0, "db": -18.0, "confidence": "implementation_verified", "source": "scratch/test_eq_ch90_band0.py"},
            {"normalized": 0.5, "db": 0.0, "confidence": "implementation_verified", "source": "scratch/eq_diagnosis.json flat EQ default"},
            {"normalized": 1.0, "db": 18.0, "confidence": "hypothesis", "source": "extrapolated from max limit"}
        ]
                
        eq_calib_path = kb_fl_api / "mixer_eq_calibration.json"
        if eq_calib_path.exists():
            with open(eq_calib_path, "r") as f:
