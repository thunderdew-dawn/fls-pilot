...
def do_propose(bridge):
    ...
    print("playing: %s   gathered in %.2fs   (peaks %s)"
          % (snap["playing"], time.time() - t0,
             "sustained" if snap["playing"] else "skipped/stopped"))
    for n in plan["notes"]:
        print("NOTE:", n)
    print()
    if not plan["plans"]:
    ...
    for p in plan["plans"]:
        ...
        print("     why: %s" % p.get("reason", ""))
        if p.get("actionable"):
            print("     exact write: %s %s" % (p["command"], p["params"]))
...
def do_apply(bridge, fix_id):
    ...
    res = safety.safe_write(
        bridge, tool=plan["tool"], scope=plan["scope"],
        command=plan["command"], params=plan["params"],
    ...
    print("  before         : vol_db=%s  vol_norm=%s" % (before.get("vol_db"), before.get("vol_norm")))
    print("  applied (fresh): vol_db=%s  vol_norm=%s" % (fresh.get("vol_db"), fresh.get("vol_norm")))
    tgt = plan.get("target_fader_db")
    if fresh.get("vol_db") is not None and tgt is not None:
        ok = abs(fresh["vol_db"] - tgt) <= 0.6
        print("  readback %s target %.1f dB" % ("MATCHES" if ok else "DIFFERS from", tgt))
...
def do_rollback(bridge):
    ...
    if scope.startswith("mixer_track:"):     # fresh read (avoids same-tick stale echo)
        ...
        print("  restored (fresh): %s  vol_db=%s  vol_norm=%s"
