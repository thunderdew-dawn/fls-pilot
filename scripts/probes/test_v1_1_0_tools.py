...
from fl_studio_mcp.protocol import CMD_CHANNEL_SET_VOLUME, CMD_CHANNEL_SET_NAME, CMD_CHANNEL_SET_TARGET

# Import tools modules
from fl_studio_mcp.tools import channels, routing, project_organizer, project_doctor, mixer_core
...
def run_tests():
    ...
    res1 = safety.safe_write(
        bridge, tool="test_setup", scope="channel:0",
        command=CMD_CHANNEL_SET_NAME, params={"channel": 0, "name": "Channel 1"},
        build_restore=lambda b: {"command": CMD_CHANNEL_SET_NAME, "params": {"channel": 0, "name": b.get("name", "Sampler")}}
    )
    res2 = safety.safe_write(
        bridge, tool="test_setup", scope="channel:0",
        command=CMD_CHANNEL_SET_VOLUME, params={"channel": 0, "value": 0.9, "unit": "normalized"},
        build_restore=lambda b: {"command": CMD_CHANNEL_SET_VOLUME, "params": {"channel": 0, "value": b.get("vol_norm", 0.78), "unit": "normalized"}}
    )
    res3 = safety.safe_write(
        bridge, tool="test_setup", scope="channel:0",
        command=CMD_CHANNEL_SET_TARGET, params={"channel": 0, "track": 0},
        build_restore=lambda b: {"command": CMD_CHANNEL_SET_TARGET, "params": {"channel": 0, "track": b.get("target_fx_track", 1)}}
    )
    ...
    try:
        ...
        res = tools["fl_apply_naming_standard"](style="psytrance", rules=[{"type": "channel", "index": 0, "name": "PSY_KICK"}])
