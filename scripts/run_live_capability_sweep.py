...
def main() -> int:
    ...
    rc = 0
    rc |= _run("Priority 1/2 live smoke", [py, "scripts/probes/test_priority12_live.py"])
    rc |= _run("Patterns & playlist live", [py, "scripts/probes/test_pattern_playlist_live.py"])
    rc |= _run("Mixer live", [py, "scripts/probes/test_mixer_live.py"])
    rc |= _run("Step sequencer live", [py, "scripts/probes/test_step_sequencer_live.py"])
    rc |= _run(
        "Plugin param live probe (track 49/50)", [py, "scripts/probes/test_plugin_param_live.py"]
    )
    rc |= _run(
        "Documented API false-positive probes",
        [py, "scripts/probes/probe_documented_api_live.py"],
    )
    rc |= _run(
        "Native EQ type mapping probe",
        [py, "scripts/probes/probe_native_eq_type_live.py"],
    )
    rc |= _run(
        "Targeted effect plugin probes (track 49/50)",
        [py, "scripts/probes/test_effect_targets_live.py"],
    )
