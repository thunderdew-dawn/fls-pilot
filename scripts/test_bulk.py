...
def main() -> int:
    check("category 'drums' -> Kick + Snare",
    ...
    check("unknown category 'trumpet' -> literal name match (6)",
          resolve_targets(TRACKS, category="trumpet") == {6})
    check("Master (index 0) is never selected",
          0 not in resolve_targets(TRACKS, category="master")
          and 0 not in resolve_targets(TRACKS, names=["master", 0]))
    check("category + names union", resolve_targets(TRACKS, category="drums", names=[5]) == {1, 2, 5})
