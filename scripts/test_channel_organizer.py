...
def main() -> int:
    ...
    check(
        "target restore uses prior mixer target",
        restore
        == {
    ...
    try:
        ...
        bridge = FakeBridge()
        check(
            "free mixer finder skips targeted track and named bus",
            channel_tools._find_free_mixer_track(bridge, start_track=1) == 2,
        )
        check(
            "plugin list checked for candidate",
            bridge.calls[-1] == (protocol.CMD_PLUGIN_LIST, {"track": 2}),
        )
    finally:
