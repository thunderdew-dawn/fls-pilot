...
from fl_studio_mcp.tools.color import (  # noqa: E402
    COLOR_NAMES, parse_color, _resolve_channels,
)
...
def main() -> int:
    ...
    # hex forms
    check("'#33A1FF' -> (51,161,255)", parse_color("#33A1FF") == (51, 161, 255), str(parse_color("#33A1FF")))
    ...
    ok = all(isinstance(v, tuple) and len(v) == 3 and all(isinstance(c, int) and 0 <= c <= 255 for c in v)
    ...
    tracks = [{"index": 0, "name": "Master"}, {"index": 1, "name": "Kick"},
              {"index": 2, "name": "Snare"}, {"index": 3, "name": "Lead Vocal"},
              {"index": 4, "name": "Bass"}]
    ...
    chans = [{"index": 0, "name": "Kick"}, {"index": 1, "name": "Clap"}, {"index": 2, "name": "Bass"}]
