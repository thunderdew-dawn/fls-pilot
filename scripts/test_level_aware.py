...
def param_strings(bridge, track, slot, names):
    ...
    for nm in names:
        if nm in P:
            out[nm] = bridge.call(protocol.CMD_PLUGIN_GET_PARAM,
                                  {"track": track, "slot": slot, "param": P[nm]["i"]}).get("s")
    return out
...
def check_apply(tag, r, offset):
    ...
    lv = r.get("level", {})
    print("  %s: matched=%s measured=%s chosen_thr=%s | set=%s"
          % (tag, lv.get("matched"), lv.get("measured"), lv.get("chosen_threshold_db"), r.get("set")))
    if lv.get("matched"):
        peak = (lv.get("measured") or {}).get("peak_db")
        check("%s level-matched: chosen ~= peak - %g" % (tag, offset),
              approx(lv.get("chosen_threshold_db"), (peak or 0) - offset, 1.0),
              "peak=%s chosen=%s" % (peak, lv.get("chosen_threshold_db")))
    else:
        check("%s preset fallback note present" % tag, bool(r.get("note")), str(r.get("note")))
...
def main() -> int:
    ...
    if not bridge.is_alive():
        print("Bridge not alive."); return 1
    m = build_server()
    ...
    r9 = call("fl_apply_compression_intent",
              {"track": LIM_TRACK, "slot": LIM_SLOT, "intent": "heavy_vocal_compression",
               "intensity": 0.6, "level_aware": True})
    check("Limiter plugin matched", "limiter" in (r9.get("plugin") or "").lower(), r9.get("plugin"))
    check_apply("Limiter", r9, offset=12.0)

    r8 = call("fl_apply_compression_intent",
              {"track": PROC_TRACK, "slot": PROC_SLOT, "intent": "glue_drums",
               "intensity": 0.5, "level_aware": True})
    check("Pro-C plugin matched", "pro-c" in (r8.get("plugin") or "").lower(), r8.get("plugin"))
    check("Pro-C Style set to Bus", (r8.get("readback", {}).get("Style") or "").strip().lower() == "bus",
    ...
    check("both rollbacks reverted compression",
          rb1 == "apply_compression_intent" and rb2 == "apply_compression_intent", "%s %s" % (rb1, rb2))
    post9 = param_strings(bridge, LIM_TRACK, LIM_SLOT, LIM_PARAMS)
    post8 = param_strings(bridge, PROC_TRACK, PROC_SLOT, PROC_PARAMS)
    check("Limiter params restored", post9 == pre9, "pre=%s post=%s" % (pre9, post9))
    check("Pro-C params restored", post8 == pre8, "pre=%s post=%s" % (pre8, post8))
