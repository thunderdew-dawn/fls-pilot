...
def main() -> int:
    ...
    print("bpm_used=%s note_count=%s confidence=%s" % (r["bpm_used"], r["note_count"], r["confidence"]))
    print("\ndetected notes (name @ start_sec, dur, prob):")
    for n in r["notes"]:
        print("  %-4s @ %.2fs  dur %.2fs  prob %.2f" % (n["name"], n["start_sec"], n["dur_sec"], n["voiced_prob"]))
    ...
    print("\nscale pitch-classes %s ; detected %s ; covered %d/7" % (SCALE_PC, detected_pc, len(covered)))

    check("found a sensible note count (5-14 for 8 notes)", 5 <= r["note_count"] <= 14, "got %d" % r["note_count"])
    check("recovers >=6/7 C-major pitch classes", len(covered) >= 6, "covered=%s" % covered)
    check("no out-of-scale pitch classes", all(pc in SCALE_PC for pc in detected_pc),
          "detected=%s" % detected_pc)
