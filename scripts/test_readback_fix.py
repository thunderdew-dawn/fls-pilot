...
def vol(bridge, mt, value, unit):
    return safety.safe_write(
        bridge, tool="mixer_set_volume", scope="mixer_track:%d" % mt,
        command=protocol.CMD_MIXER_SET_VOLUME, params={"track": mt, "value": value, "unit": unit},
...
def main() -> int:
    ...
    r = vol(b, mt, -6.0, "db")
    a = r.get("after") or {}
    check("volume after is FRESH (~ -6 dB; stale would read %.1f)" % o_db,
          a.get("vol_db") is not None and abs(a["vol_db"] - (-6.0)) <= 0.6,
          "after vol_db=%s" % a.get("vol_db"))
    vol(b, mt, o_norm, "normalized")                                 # restore

    # 2) PAN: set 0.5, expect FRESH after (~0.5).
    r = safety.safe_write(b, tool="mixer_set_pan", scope="mixer_track:%d" % mt,
        command=protocol.CMD_MIXER_SET_PAN, params={"track": mt, "value": 0.5},
        build_restore=lambda b: {"command": protocol.CMD_MIXER_SET_PAN,
                                 "params": {"track": mt, "value": b["pan"]}})
    a = r.get("after") or {}
    check("pan after is FRESH (~0.5; stale would read %.2f)" % o_pan,
          a.get("pan") is not None and abs(a["pan"] - 0.5) <= 0.05, "after pan=%s" % a.get("pan"))
    safety.safe_write(b, tool="mixer_set_pan", scope="mixer_track:%d" % mt,
        command=protocol.CMD_MIXER_SET_PAN, params={"track": mt, "value": o_pan},
        build_restore=lambda b: {"command": protocol.CMD_MIXER_SET_PAN,
                                 "params": {"track": mt, "value": b["pan"]}})  # restore

    # 3) NAME: set a test name, expect FRESH after == test name.
    test_name = ((o_name or "Track")[:18]) + "_RB"
    r = safety.safe_write(b, tool="mixer_set_name", scope="mixer_track:%d" % mt,
        command=protocol.CMD_MIXER_SET_NAME, params={"track": mt, "name": test_name},
        build_restore=lambda b: {"command": protocol.CMD_MIXER_SET_NAME,
                                 "params": {"track": mt, "name": b["name"]}})
    a = r.get("after") or {}
    check("name after is FRESH (== %r; stale would read %r)" % (test_name, o_name),
          a.get("name") == test_name, "after name=%r" % a.get("name"))
    safety.safe_write(b, tool="mixer_set_name", scope="mixer_track:%d" % mt,
        command=protocol.CMD_MIXER_SET_NAME, params={"track": mt, "name": o_name},
        build_restore=lambda b: {"command": protocol.CMD_MIXER_SET_NAME,
                                 "params": {"track": mt, "name": b["name"]}})  # restore
    ...
    try:
        ...
        r = safety.safe_write(b, tool="channel_set_volume", scope="channel:%d" % ch,
            command=protocol.CMD_CHANNEL_SET_VOLUME, params={"channel": ch, "value": -6.0, "unit": "db"},
            build_restore=lambda b: {"command": protocol.CMD_CHANNEL_SET_VOLUME,
                                     "params": {"channel": ch, "value": b["vol_norm"], "unit": "normalized"}})
        a = r.get("after") or {}
        check("channel volume after is FRESH (~ -6 dB; stale would read %.1f)" % c_db,
              a.get("vol_db") is not None and abs(a["vol_db"] - (-6.0)) <= 0.6,
              "after vol_db=%s" % a.get("vol_db"))
        safety.safe_write(b, tool="channel_set_volume", scope="channel:%d" % ch,
            command=protocol.CMD_CHANNEL_SET_VOLUME, params={"channel": ch, "value": c_norm, "unit": "normalized"},
            build_restore=lambda b: {"command": protocol.CMD_CHANNEL_SET_VOLUME,
                                     "params": {"channel": ch, "value": b["vol_norm"], "unit": "normalized"}})
    except Exception as e:
        print("  [SKIP] channel volume test -- %s: %s" % (type(e).__name__, e))
    ...
    check("rollback still works (track back to original norm)",
          rb.get("ok") and abs(fresh["vol_norm"] - o_norm) <= 0.001,
          "now vol_norm=%s (orig %s)" % (fresh["vol_norm"], o_norm))

    # final: everything restored?
    fin = b.call(protocol.CMD_MIXER_GET_TRACK, {"index": mt})
    check("project left UNCHANGED (vol/pan/name restored)",
          abs(fin["vol_norm"] - o_norm) <= 0.001 and abs(fin["pan"] - o_pan) <= 0.001
          and fin["name"] == o_name,
          "vol_norm=%s pan=%s name=%r" % (fin["vol_norm"], fin["pan"], fin["name"]))
