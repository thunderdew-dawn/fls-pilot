...
_TOOL = {
    "eq": "fl_apply_eq_intent",
    "comp": "fl_apply_compression_intent",
    "reverb": "fl_apply_reverb_intent",
}
_NEEDS = {
    "eq": "a Parametric EQ (e.g. Fruity Parametric EQ 2)",
...
RECIPES = {
    ...
    "drum_bus": [
        ("glue", "comp", "glue_drums"),
        ("low cleanup", "eq", "remove_mud"),
        ("air", "eq", "add_air"),
    ],
    "bass": [
        ("low-mid control", "eq", "remove_mud"),
        ("compression", "comp", "gentle_compression"),
    ],
    "master": [("glue", "comp", "glue_drums"), ("air", "eq", "add_air")],
}
...
def describe():
    ...
    return {
        name: [{"role": r, "needs": _NEEDS[k], "intent": i} for r, k, i in steps]
...
def plan_chain(chain_type, plugins):
    ...
    if recipe is None:
        return {
            "ok": False,
            "error": f"unknown chain_type {chain_type!r}",
            "available": available(),
        }
    steps, missing = [], []
        ...
        steps.append(
            {
                ...
                "plugin": match.get("name"),
                "intent": intent,
            }
        )
    return {
        "ok": True,
        "chain_type": (chain_type or "").lower(),
        "steps": steps,
        "missing": missing,
    }
