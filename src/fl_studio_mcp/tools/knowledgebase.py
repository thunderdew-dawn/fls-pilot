...
def kb_get_parameter_spec(api_function: str) -> str:
    ...
    if calib.exists():
        try:
            ...
            for entry in data:
                if entry.get("api_setter") == api_function or entry.get("api_getter") == api_function:
...
def kb_record_verified_finding(
    json_path: str,
    new_mapping_entry: str,
    confidence: str
) -> str:
    """Safely update a structured JSON file (e.g., fl_api/mixer_eq_calibration.json) with a new mapping."""
    if confidence not in ["implementation_verified", "cross_platform_verified", "measured_repeated", "measured_once"]:
