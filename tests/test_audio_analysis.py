...
def click_track(bpm, secs=8.0):
    ...
    for i in range(0, n, period):
        m = min(blen, n - i)
        y[i : i + m] += (np.random.rand(m).astype(np.float32) * 2 - 1) * np.hanning(m).astype(
            np.float32
        )
    return y
...
def main() -> int:
    ...
    bpm = a["tempo_bpm"]
    check(
        "tempo ~= 100 (or half/double 50/200)",
        near(bpm, 100, 4) or near(bpm, 50, 3) or near(bpm, 200, 6),
        f"got {bpm}",
    )
    ...
    check(
        "tonic is C (or A, relative minor)",
        tonic in ("C", "A"),
        f"got {tonic} {mode} (conf {conf})",
    )
