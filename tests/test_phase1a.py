...
def main() -> int:
    ...
    for t in (0, 1, 6):
        _, ok = step(
            "mixer_get_track index=%d" % t,
            lambda t=t: bridge.call(CMD_MIXER_GET_TRACK, {"index": t}),
        )
        all_ok &= ok

    _, ok = step(
        "channel_list (ALL pages)", lambda: fetch_all_pages(bridge, CMD_CHANNEL_LIST, "channels")
    )
    all_ok &= ok

    _, ok = step("channel_get index=0", lambda: bridge.call(CMD_CHANNEL_GET, {"index": 0}))
    all_ok &= ok
